...
struct heart_rate_source {
	obs_source_t *source;
	gs_texrender_t *texrender; // buffer in GPU where rendering operations are performed
	gs_stagesurf_t *stagesurface; // facilitates transferring rendered textures from the GPU to CPU
};

static void processBGRAData(uint8_t *data, uint32_t width, uint32_t height, uint32_t linesize) {
    ...
    obs_log(LOG_INFO, "Average B: %f, G: %f, R: %f, A: %f\n", averageB, averageG, averageR, averageA);
}

static bool getBGRAFromStageSurface(struct heart_rate_source *hrs, uint32_t &width, uint32_t &height) {
	...
	obs_log(LOG_INFO, "--------RETRIEVING BASE DIMENSIONS OF TARGET SOURCE!!!!!!!!");
	...
	obs_log(LOG_INFO, "--------FINISHED RESETTING THE TEXTURE RENDERER!!!!!!!!");
	...
	gs_clear(GS_CLEAR_COLOR, &background, 0.0f, 0); // Clears color/depth/stencil buffers 
