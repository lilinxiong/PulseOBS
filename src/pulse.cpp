...
obs_properties_t* heart_rate_source_properties(void* data)
{
    ...
    // Add properties
    obs_properties_add_text(props, "heart_rate_device", "Webcam Device", OBS_TEXT_DEFAULT);

    // Populate available video devices
    obs_property_t* devices_property = obs_properties_add_list(props, "heart_rate_device_list", "Choose Webcam", OBS_COMBO_TYPE_EDITABLE, OBS_COMBO_FORMAT_STRING);
    ...
    for (obs_source_t *source : device_references) {
        ...
        obs_property_list_add_string(devices_property, source_name, source_name);
    }
