...
private:
	...
	std::vector<std::vector<double>> frame_data =
		{}; // Store the average RGB values for each frame

	std::vector<std::vector<double>>
	moving_average(const std::vector<std::vector<double>> &rgb, int n = 3);
	...
	std::vector<std::vector<double>>
	magnify_colour_ma(const std::vector<std::vector<double>> &rgb,
			  double delta = 50, int n_bg_ma = 60,
			  int n_smooth_ma = 3);

	double
	Welch_cpu_heart_rate(const std::vector<std::vector<double>> &bvps,
			     double fps, int num_data_points, int nfft = 16);
